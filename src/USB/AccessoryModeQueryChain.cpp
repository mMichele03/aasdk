...
void AccessoryModeQueryChain::start(DeviceHandle handle, Promise::Pointer promise) {
    strand_.dispatch([this, self = this->shared_from_this(), handle = std::move(handle), promise = std::move(promise)]() mutable {
        if (promise_ != nullptr) {
            ...
            queryPromise->then([this, self = this->shared_from_this()](IUSBEndpoint::Pointer usbEndpoint) mutable { this->protocolVersionQueryHandler(std::move(usbEndpoint)); },
...
void AccessoryModeQueryChain::protocolVersionQueryHandler(IUSBEndpoint::Pointer usbEndpoint) {
    auto queryPromise = IAccessoryModeQuery::Promise::defer(strand_);
    queryPromise->then([this, self = this->shared_from_this()](IUSBEndpoint::Pointer usbEndpoint) mutable { this->manufacturerQueryHandler(std::move(usbEndpoint)); },
                       [this, self = this->shared_from_this()](const error::Error& e) mutable {
                           promise_->reject(e);
                           promise_.reset();
                       });
...
void AccessoryModeQueryChain::manufacturerQueryHandler(IUSBEndpoint::Pointer usbEndpoint) {
    auto queryPromise = IAccessoryModeQuery::Promise::defer(strand_);
    queryPromise->then([this, self = this->shared_from_this()](IUSBEndpoint::Pointer usbEndpoint) mutable { this->modelQueryHandler(std::move(usbEndpoint)); },
                       [this, self = this->shared_from_this()](const error::Error& e) mutable {
                           promise_->reject(e);
                           promise_.reset();
                       });
...
void AccessoryModeQueryChain::modelQueryHandler(IUSBEndpoint::Pointer usbEndpoint) {
    auto queryPromise = IAccessoryModeQuery::Promise::defer(strand_);
    queryPromise->then([this, self = this->shared_from_this()](IUSBEndpoint::Pointer usbEndpoint) mutable { this->descriptionQueryHandler(std::move(usbEndpoint)); },
                       [this, self = this->shared_from_this()](const error::Error& e) mutable {
                           promise_->reject(e);
                           promise_.reset();
                       });
...
void AccessoryModeQueryChain::descriptionQueryHandler(IUSBEndpoint::Pointer usbEndpoint) {
    auto queryPromise = IAccessoryModeQuery::Promise::defer(strand_);
    queryPromise->then([this, self = this->shared_from_this()](IUSBEndpoint::Pointer usbEndpoint) mutable { this->versionQueryHandler(std::move(usbEndpoint)); },
                       [this, self = this->shared_from_this()](const error::Error& e) mutable {
                           promise_->reject(e);
                           promise_.reset();
                       });
...
void AccessoryModeQueryChain::versionQueryHandler(IUSBEndpoint::Pointer usbEndpoint) {
    auto queryPromise = IAccessoryModeQuery::Promise::defer(strand_);
    queryPromise->then([this, self = this->shared_from_this()](IUSBEndpoint::Pointer usbEndpoint) mutable { this->uriQueryHandler(std::move(usbEndpoint)); },
                       [this, self = this->shared_from_this()](const error::Error& e) mutable {
                           promise_->reject(e);
                           promise_.reset();
                       });
...
void AccessoryModeQueryChain::uriQueryHandler(IUSBEndpoint::Pointer usbEndpoint) {
    auto queryPromise = IAccessoryModeQuery::Promise::defer(strand_);
    queryPromise->then([this, self = this->shared_from_this()](IUSBEndpoint::Pointer usbEndpoint) mutable { this->serialQueryHandler(std::move(usbEndpoint)); },
                       [this, self = this->shared_from_this()](const error::Error& e) mutable {
                           promise_->reject(e);
                           promise_.reset();
                       });
...
void AccessoryModeQueryChain::serialQueryHandler(IUSBEndpoint::Pointer usbEndpoint) {
    auto queryPromise = IAccessoryModeQuery::Promise::defer(strand_);
    queryPromise->then([this, self = this->shared_from_this()](IUSBEndpoint::Pointer usbEndpoint) mutable { this->startQueryHandler(std::move(usbEndpoint)); },
                       [this, self = this->shared_from_this()](const error::Error& e) mutable {
                           promise_->reject(e);
                           promise_.reset();
                       });
